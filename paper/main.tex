...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Function{fr\'echet\_next}{$a: \mathbb{R}^Q$, $x: \mathbb{R}^{Q}$} $\to \mathbb{R}^Q$
            \State $a_{2 \ldots Q} \gets \min\{ a_{1 \ldots Q},\, a_{2 \ldots Q+1} \}$ \label{alg:fast:min}
...
\begin{algorithm}
    \caption{In-place variant of Alg.~\ref{alg:vanilla}. This variant directly maps a given distance matrix $d$ to the corresponding Fr\'echet distance. Note that this variant has no explicit branching points.}
    \label{alg:inplace}
    \begin{algorithmic}[1]
        \Function{fr\'echet\_distance}{$d: \mathbb{R}^{P \times Q}$} $\to \mathbb{R}$
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Function{levenshtein\_distance}{$p: \mathfrak{C}^P$, $q: \mathfrak{C}^Q$} $\to \mathbb{N}$
            \State $v: \mathbb{N}^Q \gets$ \Call{levenshtein\_min$\backslash$}{$f_\mathfrak{C}(p_1, q) + [0, \ldots, Q-1]$}
            \Statex
            \For{$i \gets 2,P$}
